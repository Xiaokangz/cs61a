...
CREATE TABLE schedule AS
  with sch(route, sum_price, last, len) as (
    select "SFO", 0, "SFO", 0 union
    select route || ", " || arrival, sum_price + price, arrival, len + 1 from sch, flights where last = departure and len < 2)
  SELECT route, sum_price from sch where last = "PDX" and len <= 2 order by sum_price;
...
CREATE TABLE shopping_list AS
  with best_opt(name, opt) as (
    select name, min(MSRP / rating) from products group by category) 
  SELECT name, store from best_opt, lowest_prices where name = item;
