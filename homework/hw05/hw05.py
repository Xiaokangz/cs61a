...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
        ...
        temp += [replace_leaf(item, old, new) for item in branches(t)]
    return temp
...
def quadratic(x, a, b, c):
    ...
    "*** YOUR CODE HERE ***"
    extremenum = -b / (2 * a)
    l, u, e = map(lambda x: a * x * x + b * x + c,(lower_bound(x), upper_bound(x), extremenum))
    if extremenum >= lower_bound(x) and extremenum <= upper_bound(x):
...
def polynomial(x, c):
    ...
    def add_df(coeff, k, df):
        return lambda x: k * coeff * pow(x, k - 1) + df(x)
    def add_ddf(coeff, k, ddf):
    ...
    for k, coeff in enumerate(c):
        f = add_fn(coeff, k, f)
        if k > 0:
            df = add_df(coeff, k, df)
        if k > 1:
            ddf = add_ddf(coeff, k, ddf)
    lower, upper = lower_bound(x), upper_bound(x)
    steps = (upper - lower) / 20
    xn = [lower + k * steps for k in range(20)]
    extremenums = [find_zero(df, ddf, n) for n in xn]
    ns = [n for n in extremenums if n > lower and n < upper] + [lower, upper]
...
def find_zero(f, df, guess = 1):
    ...
    def newton_update(x):
        return x - f(x) / df(x)
    return improve(newton_update, near_zero, guess)