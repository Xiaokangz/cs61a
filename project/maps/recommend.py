...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    xx = sum([(x - mean(xs))**2 for x in xs])
    yy = sum([(y - mean(ys))**2 for y in ys])
    xy = 0
    for i in range(len(restaurants)):
        xy += ((xs[i] - mean(xs)) * (ys[i] - mean(ys)))
    b = xy / xx
    a = mean(ys) - b * mean(xs)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    return find_predictor(user, reviewed, max(feature_fns, key = lambda f: find_predictor(user, reviewed, f)[1]))[0]
    # END Question 8
