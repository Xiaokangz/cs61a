...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(self, operands.map(lambda e: scheme_eval(e, env)), env)
        "*** YOUR CODE HERE ***"
        # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    sub_bool = True
    while expressions.second is not nil:
        sub_bool = scheme_eval(expressions.first, env)
        if not scheme_truep(sub_bool):
            return sub_bool
        expressions = expressions.second
    return scheme_eval(expressions.first, env, True)
    "*** YOUR CODE HERE ***"
