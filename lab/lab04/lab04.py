...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    "*** YOUR CODE HERE ***"
    i = 0
    while i < max_cols - num_connect + 1:
        j = 0
        while j < num_connect and get_piece(board, row, i + j) == player:
            j = j + 1
        if j == num_connect:
            return True
        i = i + 1
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    "*** YOUR CODE HERE ***"
    i = 0
    while i < max_rows - num_connect + 1:
        j = 0
        while j < num_connect and get_piece(board, i + j, col) == player:
            j = j + 1
        if j == num_connect:
            return True
        i = i + 1
